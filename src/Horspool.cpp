/*
 * Horspool.cpp
 *
 *  Created on: Apr 2, 2016
 *      Author: note
 */

#include "Horspool.h"
int bmBc[NO_OF_CHARS];
Horspool::Horspool() {
	// TODO Auto-generated constructor stub

}

Horspool::~Horspool() {
	// TODO Auto-generated destructor stub
}

void Horspool::HORSPOOL_preBmBc(char *x, int m) {
   int i;
   for (i = 0; i < NO_OF_CHARS; ++i)
      bmBc[i] = m;
   for (i = 0; i < m - 1; ++i){
      bmBc[x[i]] = m - i - 1;
      //cout << "x: "<< x[i] << endl;
      //cout << "bmBc: "<< bmBc[x[i]] << endl;

      //cout << "bmBc[x[i]]"<< bmBc[x[i]] << endl;
   }
}

bool Horspool::getMatch()
{
  return Match ;
}

void Horspool::Run_HORSPOOL(char *x, int m, char *y, int n,string id,string column) {
   int j;
   char c;

   /* Preprocessing */
   //HORSPOOL_preBmBc(x, m);
   /* Searching */
   j = 0;
  /*while (j <= n - m) {
	   //cout << "J: "<< j << endl;
      c = y[j + m - 1];
      if (x[m - 1] == c && memcmp(x, y + j, m-1 ) == 0){
    	  cout << "Column name: "<< column << " | Pattern ID: "<< id <<" | Found at: "<< j << endl;
    	  break;
      }else{
       j += bmBc[c];
      }
   }*/
   j  = m - 1; // Position of the pattern's right end
   while ( j <= n-1) {
       int k = 0; // Number of matched characters
       while (k <= m-1 && x[m-1-k] == y[j-k])
           k++;
       if (k == m){
    	   cout << "Column name: "<< column << " | Pattern ID: "<< id <<" | Found at: "<< j - m + 1 << endl;
       	   break;
       }else{
           j = j + bmBc[(size_t)y[j]];
       }
   }
   //delete [] bmBc;
}


void Horspool::search( char *txt,  char *pat , string id,string column){
    int m = strlen(pat);
    int n = strlen(txt);


	Run_HORSPOOL(pat,m,txt,n,id,column);
}



